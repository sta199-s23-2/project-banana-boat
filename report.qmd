...
Is there a significant difference in the mean annual consumption across all states of coal in the United States changed before and after the the 21st century.

```{r}

south |>
  group_by(State) |>
  summarize(mean_consumption = mean(prop_renewable))

null_dist <- south |>
  specify(response = prop_renewable) |>
